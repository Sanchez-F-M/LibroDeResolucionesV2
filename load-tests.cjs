...
const LOAD_TEST_CONFIG = {
    concurrent_users: [1, 3, 5],  // Reducido para no sobrecargar
    duration_seconds: 30,         // Reducido para pruebas más rápidas
    request_interval_ms: 2000,    // Más espaciado
    timeout: 15000
};
    ...
    async makeRequest(userId) {
        ...
        try {
            ...
            if (requestType < 0.5) {
                ...
                // 50% Diagnose
                await axios.get(`${BACKEND_URL}/diagnose`, { 
                    timeout: LOAD_TEST_CONFIG.timeout 
                });
            }
...
async function runLoadTests() {
    ...
    console.log(`⏱️ Duración por prueba: ${LOAD_TEST_CONFIG.duration_seconds} segundos`);
    console.log(`📡 Intervalo entre requests: ${LOAD_TEST_CONFIG.request_interval_ms}ms`);
    console.log('');

    const loadTestRunner = new LoadTestRunner();
    const durationMs = LOAD_TEST_CONFIG.duration_seconds * 1000;
    ...
    for (const userCount of LOAD_TEST_CONFIG.concurrent_users) {
        ...
        console.log('⏸️ Pausa de 15 segundos entre pruebas...');
